...
describe('BoardComponent — X-Change exchange is not a committed cross', () => {
  let component: BoardComponent;
  ...
  // A passive player who crossed a normal cell: a committed white+white selection.
  function passiveWithCommittedCross(): GameState {
    return makeState({
      turnState: {
        activePlayerId: OTHER_ID,
        phase: TurnPhase.PASSIVE_MOVE,
        passivePlayerQueue: [PLAYER_ID],
        pendingCrosses: { [PLAYER_ID]: ['normal-cell'] },
      },
    });
  }

  // A passive player who crossed an X-Change cell: the exchange is pending (effectiveWhiteWhite is
  // set) and they still owe the real follow-up cross.
  function passiveWithPendingExchange(): GameState {
    return makeState({
      turnState: {
        activePlayerId: OTHER_ID,
        phase: TurnPhase.PASSIVE_MOVE,
        passivePlayerQueue: [PLAYER_ID],
        pendingCrosses: { [PLAYER_ID]: ['xc-cell'] },
        effectiveWhiteWhite: { [PLAYER_ID]: 12 },
      },
    });
  }

  it('treats a pending X-Change exchange as NOT a committed passive cross', () => {
    component.gameState.set(passiveWithPendingExchange());
    expect(component.hasXChangePending()).toBe(true);
    // The exchange must not count as committed — otherwise the closure modal PASSes the turn on it.
    expect(component.hasPendingPassiveCross()).toBe(false);
  });

  it('treats a real (non-exchange) pending passive cross as committed', () => {
    component.gameState.set(passiveWithCommittedCross());
    expect(component.hasXChangePending()).toBe(false);
    expect(component.hasPendingPassiveCross()).toBe(true);
  });
});
